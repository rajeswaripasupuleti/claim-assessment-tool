...
import { captureRequest, shouldCaptureRequest, } from "../utils/capture-request.util.js";
...
let HttpObserveAgentService = class HttpObserveAgentService {
    ...
    startHttpRequestTracing(req, res, done) {
        ...
        this.asyncLocalStorage.run(store, () => {
            ...
            // about.
            const redactedUrl = redactUrlQuery(req.url, this.operationTraceRegistry.getRedactor());
    ...
    endHttpRequestTracing(req, res) {
        ...
        setTimeout(async () => {
            ...
            if (shouldCaptureRequest(snapshot, this.options.http?.capture)) {
                const captured = captureRequest(req, this.options.http?.capture, this.operationTraceRegistry.getRedactor());
                if (captured) {
                    snapshot.request = captured;
                }
            }

...
describeRefusal) => {
    ...
    const post = (body) => fetch(telemetryUrl, { method: "POST", headers, body });
    const postBatch = async (json) => post(await gzipAsync(json));
    /**
    ...
    const sendInstrumentationJsonData = async () => {
        ...
        try {
            ...
            try {
                ...
                parentPort.postMessage(endpointProblem
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    ...
                    const why = typeof describeRefusal === "function"
                    ...
                    parentPort.postMessage(`Error: Telemetry rate-limited (429) - ${why}. ` +
             ...
             */
            const accepted = await response.json().catch(() => undefined);
            const degraded = typeof accepted === "object" &&
                accepted !== null &&
                accepted.degraded === true;
            ...
            const causeText = cause instanceof Error || typeof cause === "string"
                ? String(cause)
                : undefined;
            parentPort.postMessage(`Error: Telemetry worker failed while sending a batch to ${telemetryUrl} - ` +
                String(err) +

...
import { CONFIGURATION_TOKEN, VALIDATED_ENV_PROPNAME, } from './config.constants.js';
    ...
    updateInterpolatedEnv(propertyPath, value) {
        ...
        for (const envFilePath of this.envFilePaths) {
            if (fs.existsSync(envFilePath)) {
                config = Object.assign(this.parser(fs.readFileSync(envFilePath)), config);
            }
        }
        const escapedKey = propertyPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const regex = new RegExp(`\\$\\{${escapedKey}\\}|\\$${escapedKey}(?!\\w)`, 'g');
...
ConfigService = __decorate([
    Injectable(),
    __param(0, Optional()),
    __param(0, Inject(CONFIGURATION_TOKEN)),

...
import { DiscoveryModule, DiscoveryService, HttpAdapterHost, MetadataScanner, ModulesContainer, } from "@nestjs/core";
 ...
 */
const MISSING_ASYNC_OPTIONS_PROVIDER = 'ObserveModule.forRootAsync() requires one of "useFactory", "useClass" or "useExisting".';
export function createObserveModule(options = {}) {
    var ObserveModule_1;
    ...
    const operationTraceRegistry = new OperationTraceRegistry(asyncLocalStorage, options.sourceContext);
    let ObserveModule = ObserveModule_1 = class ObserveModule {
        httpAdapterHost;
        asyncLocalStorage;
        options;
        logger = new Logger(ObserveModule_1.name);
        constructor(httpAdapterHost, asyncLocalStorage, options) {
            this.httpAdapterHost = httpAdapterHost;
            this.asyncLocalStorage = asyncLocalStorage;
            this.options = options;
        }
        static forRoot(observeOpts) {
            return {
                global: true,
                module: ObserveModule_1,
                providers: [
                    {
        ...
        static createAsyncOptionsProvider(asyncOptions) {
            ...
            const optionsFactoryToken = asyncOptions.useExisting ?? asyncOptions.useClass;
    ...
    ObserveModule = ObserveModule_1 = __decorate([
        Module({
            ...
            providers: [
                ...
                },
                {
                    ...
                    useFactory: (observeOptions) => {
                        operationTraceRegistry.configureSpanCollapse(resolveSpanCollapseSettings(observeOptions?.spanCollapse));
                        operationTraceRegistry.configureRedaction(observeOptions?.redaction?.enabled === false
                            ? null
    ...
    const skipInstrumentation = (instance) => {
        try {
            return (
            ...
            options.skipInstrumentation(instance) ||
                [asyncLocalStorage, operationTraceRegistry].includes(instance) ||
    ...
    return {
        ObserveInstrument: {
            ...
            instanceDecorator: (() => {
                const decorate = createInstanceDecorator(asyncLocalStorage, operationTraceRegistry, {
                    traceIdKey: options.traceIdKey,
                    skipInstrumentation,
                });
                return (instance) => {
                    propagateTraceIdThrough(instance, () => asyncLocalStorage.getStore()?.get(options.traceIdKey));
 ...
 */
function propagateTraceIdThrough(instance, currentTraceId) {
 ...
 * on anything else.
 */
function isResolverDecoratorHost(instance) {
    const candidate = instance;
    return (typeof candidate?.decorate === "function" &&

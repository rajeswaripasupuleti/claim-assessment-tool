...
var ObserveAgentSharedBuffer_1;
import { countTraceNodes, DEGRADED_TTL_MS, isNotableSnapshot, } from "./degraded-ingest.protocol.js";
import { Inject, Injectable, Logger } from "@nestjs/common";
import { Counter, Summary } from "../custom-metrics/index.js";
import { CustomMetricsEncoder, } from "../encoders/custom-metrics.encoder.js";
import { JobSnapshotEncoder, } from "../encoders/job-snapshot.encoder.js";
import { RequestSnapshotEncoder, } from "../encoders/request-snapshot.encoder.js";
import { RuntimeMetricsEncoder, } from "../encoders/runtime-metrics.encoder.js";
...
let ObserveAgentSharedBuffer = ObserveAgentSharedBuffer_1 = class ObserveAgentSharedBuffer {
    options;
    _sharedBuffer = new SharedArrayBuffer(SHARED_BUFFER_SIZE);
    lock = new Int32Array(this._sharedBuffer, 0, 1);
    sharedBytes = new Uint8Array(this._sharedBuffer);
    encoder = new TextEncoder();
    logger = new Logger(ObserveAgentSharedBuffer_1.name);
    _mainThreadBuffer = null;
    /**
    ...
    insertRequestSnapshot(snapshot) {
        ...
        if (!snapshot.operationId) {
            if (this.options.debug) {
                this.logger.debug(`Snapshot for traceId "${snapshot.traceId}" has no operation id nor URL to fall back on. Ignoring snapshot.`);
            }
            return;
        }
        ...
        const maxPerBatch = this.options.maxTracesPerBatch ?? DEFAULT_MAX_SNAPSHOTS_PER_TRANSACTION;
        if (this._mainThreadBuffer.snapshots.length >= maxPerBatch) {
            if (this.options.debug) {
                this.logger.debug(`Max snapshots per transaction reached: ${maxPerBatch}. Ignoring snapshot.`);
            }
            return;
        }
        const encodedSnapshot = RequestSnapshotEncoder.encode(snapshot);
    ...
    insertJobSnapshot(jobSnapshot) {
        ...
        if (this._mainThreadBuffer.jobs.length >= maxPerBatch) {
            if (this.options.debug) {
                this.logger.debug(`Max job snapshots per transaction reached: ${maxPerBatch}. Ignoring snapshot.`);
            }
            return;
        }
        const encodedJobSnapshot = JobSnapshotEncoder.encode(jobSnapshot);
    ...
    addNodeRuntimeMetrics(runtimeMetrics) {
        if (!runtimeMetrics) {
            return;
        }
        if (!this._mainThreadBuffer) {
    ...
    upsertCustomMetric(metric) {
        ...
        const existingMetricIndex = this._mainThreadBuffer.custom.findIndex((m) => m.n === metric.name && m.t === metric.type);
        if (existingMetricIndex !== -1) {
            this._mainThreadBuffer.custom[existingMetricIndex] = {
                ...this._mainThreadBuffer.custom[existingMetricIndex],
                ...CustomMetricsEncoder.encode(metric),
            };
        }
        else {
            this._mainThreadBuffer.custom.push(CustomMetricsEncoder.encode(metric));
        }
    }
    /**
    ...
    pushLogs(logs) {
        ...
        for (const log of logs) {
            ...
            const entry = typeof log.text === "string" && log.text.length > MAX_LOG_ENTRY_LENGTH
    ...
    drainPendingLogs() {
        ...
        const capacity = MAX_LOGS_PER_TRANSACTION - this._mainThreadBuffer.logs.length;
        if (capacity <= 0) {
            return;
        }
        this._mainThreadBuffer.logs.push(...this._pendingLogs.splice(0, Math.min(capacity, MAX_PENDING_LOGS)));
    }
    truncateLogText(text) {
        return (text.slice(0, MAX_LOG_ENTRY_LENGTH - TRUNCATION_SUFFIX.length) +
            TRUNCATION_SUFFIX);
    }
    encodeAndWrite() {
        if (this.options.debug) {
            if (this._mainThreadBuffer?.snapshots) {
                this.logger.debug(`Encoding and writing ${this._mainThreadBuffer.snapshots.length} snapshots to shared buffer.`);
                this.logger.debug(`Operations in shared buffer: ${this._mainThreadBuffer.snapshots.map((s) => s.op)}`);
            }
        }
...
ObserveAgentSharedBuffer = ObserveAgentSharedBuffer_1 = __decorate([
    Injectable(),
    __param(0, Inject(OBSERVE_OPTIONS)),
    __metadata("design:paramtypes", [Object])
], ObserveAgentSharedBuffer);
export { ObserveAgentSharedBuffer };
